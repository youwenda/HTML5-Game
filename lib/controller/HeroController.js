...
(function(window){
	...
	p.move = function(){
		...
		var _isCollision = false, _monsterManager = this.monsterManager, _monster = null;
		for(var i = 0, l = _monsterManager.getLength(); i < l; i++){
			_monster = _monsterManager.getSprite(i);
			_isCollision = this.collisionTest(this.hero, _monster);
			_monster = null;
			if(_isCollision){
				break;
			}
		...
		if(_isCollision){
			this.hero.update(_drawXPre, _drawYPre);
			//this.stop();
			return;
		}
